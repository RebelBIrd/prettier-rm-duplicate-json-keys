...
function createSortCompareFunction(jsonSortOrder) {
  ...
  return (a, b) => {
    ...
    if (aIndex === -1 && bIndex === -1) {
      ...
      /* c8 ignore stop */
      const categorySortFunction = categorySort === null ? lexicalSort : categorySortFunctions[categorySort]
...
function createParser(parser) {
  return async (text, prettierOptions) => {
    ...
    if (!(ast.type === 'ObjectExpression' || (ast.type === 'ArrayExpression' && jsonRecursiveSort))) {
      return jsonRootAst
    }
